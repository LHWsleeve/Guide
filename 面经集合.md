...
- 线程池大小如何设置？ 线程数设置过大有什么缺点？(对于CPU密集型，尽量使用较小的线程数，过大的线程数只会徒增上下文切换次数，额外开销。对于IO密集型，要开大一点，此类型任务CPU使用率不高，可以让cpu在等待IO的时候处理别的任务。)
- **阻塞队列以及生产者消费者的实现**
...
- [MVCC 机制的实现原理](https://segmentfault.com/a/1190000012650596)
  ```html
...
```
1. 是否支持行锁：myisam只有表锁，innodb支持行锁和表级锁
2. 是否支持事务和崩溃后安全恢复：myisam强调性能，每次查询具有原子性，执行速度比innodb更快，但是不支持事务。
3. 是否支持外键：myisam不支持，innodb支持
4. 是否支持MVCC：仅innodb支持。应对高并发事务，mvcc比单纯枷锁更高效。mvcc只在可重复读和读已提交下工作。

```
- ==数据库的索引==---`如何建索引`---`索引失效`---`索引底层实现是什么`
- mysql使用索引使用**B+Tree**和**哈希索引**，一般单挑查询语句sql会自己优化使用哈希索引。
- 聚簇索引和非聚簇索引
  ```html
  聚簇索引和非聚簇索引的根本区别：
  聚簇索引（innodb）的叶子节点就是数据节点
  非聚簇索引（myisam）的叶子节点仍然是索引文件只是这个索引文件中包含指向对应数据块的指针。
  B+树有主键索引和辅助索引两种:
  主键索引： 按照表中主键的顺序构建一颗B+树，并在叶节点中存放表中的行记录数据，一个表只能有一个主键索引。
  辅助索引： 辅助索引的data域存储相应记录主键的值，而不是页的地址。通过辅助索引查找到对应的主键，然后在聚集索引中使用主键值，再走一遍主索引。（这个叫回表查询）
  因此不建议使用过长的字段作为主键，主键必须单调。
  ```
